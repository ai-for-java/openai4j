...
public class DefaultOpenAiClient extends OpenAiClient {
  ...
  private DefaultOpenAiClient(Builder serviceBuilder) {
    ...
    if (
      serviceBuilder.openAiApiKey == null && serviceBuilder.azureApiKey == null
    ) {
      throw new IllegalArgumentException(
        "openAiApiKey OR azureApiKey must be defined"
      );
    }
    if (
      serviceBuilder.openAiApiKey != null && serviceBuilder.azureApiKey != null
      ...
      throw new IllegalArgumentException(
        "openAiApiKey AND azureApiKey cannot both be defined at the same time"
      );
    }
    if (serviceBuilder.openAiApiKey != null) {
      okHttpClientBuilder.addInterceptor(
        new AuthorizationHeaderInjector(serviceBuilder.openAiApiKey)
      );
    } else {
      okHttpClientBuilder.addInterceptor(
        new ApiKeyHeaderInjector(serviceBuilder.azureApiKey)
      );
    }
  ...
  public SyncOrAsyncOrStreaming<CompletionResponse> completion(
    ...
    CompletionRequest syncRequest = CompletionRequest
      .builder()
      .from(request)
      .stream(null)
      .build();
  ...
  public SyncOrAsyncOrStreaming<ChatCompletionResponse> chatCompletion(
    ...
    ChatCompletionRequest syncRequest = ChatCompletionRequest
      .builder()
      .from(request)
      .stream(null)
      .build();
  ...
  public SyncOrAsync<EmbeddingResponse> embedding(EmbeddingRequest request) {
    return new RequestExecutor<>(
      openAiApi.embeddings(request, apiVersion),
      r -> r
    );
  }
  ...
  public SyncOrAsync<ModerationResponse> moderation(ModerationRequest request) {
    return new RequestExecutor<>(
      openAiApi.moderations(request, apiVersion),
      r -> r
    );
  }
  ...
  @Override
  public SyncOrAsync<ImageResponse> imagesGenerations(ImageRequest request) {
    return new RequestExecutor<>(
